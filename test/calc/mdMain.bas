...
Sub Main()
    Dim vLocals     As Variant
    Dim vResult     As Variant
    
    ReDim vLocals(0 To 255) As Long
    If VbPegBeginMatch(Command$ & vbCrLf, UserData:=vLocals) Then
        Do While VbPegParseStmt()
        Loop
        If VbPegEndMatch(vResult) Then
            ConsolePrint "Result: %1", vResult
        End If
    End If
End Sub
